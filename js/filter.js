...
const checkForComplianceArrays = (firstArray, secondArray) => {
...
const filterAds = () => {
  ...
  for (const ad of window.page.getSavedAds()) {
    if (
      ...
      (enabledHousingFeatures.length === 0 || checkForComplianceArrays(enabledHousingFeatures, ad.offer.features))
    ) {
